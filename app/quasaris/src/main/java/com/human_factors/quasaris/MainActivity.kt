package com.human_factors.quasaris
...
class MainActivity : AppCompatActivity() {
    ...
    private var tempMediaRecorder: MediaRecorder? = null
    ...
    private fun startTempRecording() {
        currentAudioPath = "${getExternalFilesDir(null)?.absolutePath}/temp_audio_note.3gp"
        tempMediaRecorder = MediaRecorder().apply {
            ...
            Log.e("MainActivity", "Temp recording failed", e)
            }
        }
    }

    private fun stopTempRecording() {
        tempMediaRecorder?.apply {
            try {
                stop()
            } catch (e: RuntimeException) {
                Log.e("MainActivity", "Failed to stop temp recording", e)
            }
            release()
        }
        tempMediaRecorder = null
        // Delete the temporary file as we do not need to save the recording
        File(currentAudioPath).delete()
    }
